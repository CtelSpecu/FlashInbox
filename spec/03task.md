...
**测试要点**
- 连续 10 次 create 后第 11 次被限流
- 限流后等待冷却时间可恢复
- Recover 失败 5 次后冷却时间为 60 分钟
...
**验收标准**
- [ ] 只能认领 UNCLAIMED 状态的邮箱
- [ ] Turnstile 验证必须通过
- [ ] Key 正确生成（32 字符）并哈希存储
- [ ] 状态转换为 CLAIMED
- [ ] 手动创建的地址不能被 claim
...
**测试要点**
- 错误的 Key 返回 `INVALID_CREDENTIALS`
- 不存在的邮箱返回相同错误（不区分）
- 续期后 `key_expires_at` 更新为 now + 15d
...
- [ ] 能接收入站邮件
- [ ] 正确解析收件人
- [ ] 域名检查生效（disabled 域名拒绝）
- [ ] 邮件正确存储
- [ ] 审计日志记录

**测试要点**
- 发送到未知域名的邮件被拒绝
- 发送到 disabled 域名的邮件被拒绝
- 发送到新地址自动创建 UNCLAIMED 邮箱
...
**测试要点**
- 解析含附件邮件，`has_attachments` 为 true
- 100KB 以上的正文被截断
- `text_truncated` 标志正确
...
- `<img src="http://..." />` 被替换为占位符
- `<a href="javascript:alert(1)">` 的 href 被移除
...
**测试要点**
- 优先级 10 的规则先于优先级 100 执行
- 命中 DROP 规则后邮件不存入 messages
- 命中 QUARANTINE 规则后邮件存入 quarantine
...
**验收标准**
- [ ] 域名 CRUD 操作正常
- [ ] 状态切换正确（enabled/disabled/readonly）
- [ ] 重复域名添加失败
- [ ] 审计日志记录
