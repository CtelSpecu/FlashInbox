...
- [x] URL 追踪参数正确
- [x] 所有管理功能可用
- [x] TailAdmin 布局 + shadcn/ui 组件集成正确
- [x] 响应式布局正常
- [x] 图标使用 lucide 图标集
...
> 本阶段为规划阶段：仅定义需求拆解与验收标准，不包含实现代码。

### Task 8.1: 通知与音效偏好开关（前端）
...
**任务描述**
- 在收件箱页提供“新邮件通知/提示音”开关（默认关闭）
- 权限请求必须由用户手势触发

**验收标准**
- [ ] 默认关闭，不会在页面加载时弹出权限请求
- [ ] 用户点击开启后才触发 `Notification.requestPermission()`
- [ ] 可随时关闭并立即生效

---

### Task 8.2: 新邮件系统通知（Notification API）
...
**任务描述**
- 通过收件箱刷新/轮询检测新邮件（未读数增加或新 message id）
- 页面后台时触发系统通知

**验收标准**
- [ ] 仅在用户授权且开启开关后触发通知
- [ ] 通知内容默认不包含正文（仅提示“新邮件到达”）
- [ ] 不触发外链加载或泄露邮箱地址到第三方
...
**验收标准**
- [ ] 仅在用户开启且完成一次用户手势激活后播放
- [ ] 音效时长短、音量适中，不造成打扰
- [ ] 关闭后不再播放

---

### Task 8.4: Push 通知（可选，后续）
...
**任务描述**
- Service Worker + Push API，使页面关闭仍可收到通知（Chromium/Firefox）
