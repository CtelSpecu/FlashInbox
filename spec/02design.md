...
目标：在 Chromium / Firefox 内核浏览器中支持“新邮件到达”提醒能力（系统通知 + 提示音），并保持默认关闭、用户可控、无隐私泄露。

### 8.1 触发源

- 前端以“收件箱轮询/刷新”的结果作为触发源：
  - `unreadCount` 增加
  - 或消息列表出现新 `message.id`
- 仅在用户显式开启后触发（本地偏好/账号偏好二选一，优先本地偏好实现）

### 8.2 通知（Notification API）

- 开启流程（必须用户手势触发）：
  - UI 开关 → `Notification.requestPermission()`
  - permission 为 `granted` 才允许发送系统通知
- 内容策略：
  - 默认：仅显示“新邮件到达”
  - 可选：在用户允许的情况下展示发件人/主题（不展示正文）
- 行为策略：
  - 页面不可见（`document.hidden === true`）时优先系统通知
  - 页面可见时使用站内提示（Snackbar/Badge），系统通知可选

### 8.3 提示音

- 需要用户手势激活（浏览器自动播放限制）：
  - 在用户第一次开启提示音时播放一次短音，作为“解锁音频”的手势确认
- 音效实现建议：
  - 优先 Web Audio API 生成短促提示音（无需外部音频文件）
  - 或使用内置静态音频资源（需体积控制）
- 必须支持关闭，并提供“免打扰”策略（可选）

### 8.4 Phase 规划（仅规划）

1) Phase A（In-app）：
   - 页面打开状态下的新邮件通知 + 音效（不含 Push）
2) Phase B（Background, optional）：
   - Service Worker + Push API：页面关闭仍可接收通知（需要额外推送订阅与 VAPID 配置）
3) Phase C（Advanced, optional）：
   - 通知分组、静默时间段、设备多端同步偏好、Badge 支持
